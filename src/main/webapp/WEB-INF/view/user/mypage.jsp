...
<script type="text/javascript">
	function disp_div(id,tab) {
		...
		$(".tab").removeClass("select")	//class="tab"인 태그의 select 클래스 속성 제거
		$("#"+id).show()	//minfo id="minfo"인 태그부분 보임
		$("#"+tab).addClass("select")	//id="tab1"인 태그에 select class 속성 추가
	}
	function list_disp(id) {
		$("#"+id).toggle()	//toggle() : jquery함수: 
							//보이는 경우 안보이도록, 안보이는 경우 보이도록
	}
				...
				<table class="w3-table">
					...
					<c:forEach items="${sale.itemList}" var="saleItem">
						<tr><td class="title">${saleItem.item.name}</td>
							...
							<td>
								<fmt:formatNumber value="${saleItem.quantity*saleItem.item.price}" pattern="#,###" />원</td>
						</tr>
					</c:forEach>	
				</table>
			</td>
...
<div id="minfo" class="info">
	<table class="w3-table">
		...
		<tr><td>주소</td><td>${user.userid}</td></tr>
		<tr><td>생년월일</td>
